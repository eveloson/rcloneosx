...
final class QuickBackup: SetConfigurations {
    ...
    init() {
        ...
        if self.estimatedlist != nil {
            self.sortedlist = self.configurations?.getConfigurationsSyncandCopy()?.filter({($0.value(forKey: "selectCellID") as? Int) == 1})
            guard self.sortedlist!.count > 0 else { return }
        } else {
            self.sortedlist = self.configurations?.getConfigurationsSyncandCopy()
        }
        self.sortbydays()
        self.hiddenID = nil
